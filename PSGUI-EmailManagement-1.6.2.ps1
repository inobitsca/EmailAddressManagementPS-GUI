...
Foreach ($Prim in $user.proxyaddresses) {
If ($Prim -clike "*SMTP*") {
$Pri = $Prim 
$PRi =$PRi -replace 'SMTP:',''}
}

# Show Email Address
$EmailChoice                          = New-Object system.Windows.Forms.Label
if (!$pri) {$EmailChoice.text          = "No Primary Email address has been set"}
if ($pri) {$EmailChoice.text                      = $Pri}
...
####Bulk Actions Form - FOR FUTURE USE #######
#BulkActionsForm
...
set-ADuser -Identity $CN -Add @{Proxyaddresses="SMTP:"+$newEmail}
...
if ( $Obj -eq "Group"){
Write-host $changeEmail
set-ADGroup -Identity $CN -remove @{Proxyaddresses="SMTP:"+$user.mail}
set-ADGroup -Identity $CN -remove @{Proxyaddresses="smtp:"+$NewEmail}
set-ADGroup -Identity $CN -Add @{Proxyaddresses="smtp:"+$user.mail}
set-ADGroup -Identity $CN -Add @{Proxyaddresses="SMTP:"+$NewEmail}
set-ADGroup -Identity $CN -replace @{mail=$NewEmail}
Write-Host "Primary Group Email Set" $changemail.txt
Write-host "Refreshing User Data"
$user = Get-ADObject $CN -Properties mail,proxyaddresses,msExchHideFromAddressLists
ResultsForm
}
